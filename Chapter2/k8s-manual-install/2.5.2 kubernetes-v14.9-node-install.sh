...
cat > /etc/yum.repos.d/centos7-docker-ce.repo <<-EOF
[centos7-docker-ce]
name=centos7-docker-ce
...
yum install docker-ce-${DOCKER_VERSION} -y -q
...
wget -q http://${WEB_SERVER}/k8s/${K8S_VERSION}/kubelet
...
KUBE_APISERVER="https://${K8S_MASTER}:6443"
...
FLANNEL_OPTIONS="--etcd-endpoints=https://${K8S_MASTER}:2379 -etcd-cafile=/opt/k8s/ssl/etcd-ca.pem -etcd-certfile=/opt/k8s/ssl/etcd-server.pem -etcd-keyfile=/opt/k8s/ssl/etcd-server-key.pem -etcd-prefix=/opt/k8s/network"
EOF
